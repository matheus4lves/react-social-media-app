...
function Search() {
  ...
  useEffect(() => {
    ...
    return () => document.removeEventListener("keyup", checkIfEscWasPressed);
  }, []);

  function checkIfEscWasPressed(e) {
    if (e.keyCode == 27) {
      appDispatch({ type: "searchClosed" });
    }
  }
  ...
  useEffect(() => {
    if (state.requestCount) {
      const serverResquest = Axios.CancelToken.source();
      async function fetchResults() {
        try {
          const response = await Axios.post(
            "/search",
            { searchTerm: state.searchTerm },
            { cancelToken: serverResquest.token }
          );
          setState((draft) => {
